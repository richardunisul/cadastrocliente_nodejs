...
const inserir = async (request, response) => { 
      ...
      response.status(201).json({ message: `Cliente inserido com 
    ...
    const alterar = async (request, response) => {
      ...
      if (cliente) {
        ...
        response.status(200).json( { message: `Cliente modificado com
                                clienteId: ${request.params.clienteId}` });
      } else {
    ...
    const excluir = async (request, response) => {
          ...
          if (cliente) {
           await cliente.destroy();    
            response.status(200).json( { message: `Cliente excluído com
                                          clienteId: ${cliente.clienteId}` });
          } else {
            response.status(404).json( { message: `Cliente não encontrado com 
                                    clienteId: ${request.params.clienteId}` });
          }
        };
        // Exporta as funções
